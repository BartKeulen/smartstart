...
class QLearning(Counter):
    ...
    def get_action(self, obs):
        if self.exploration == QLearning.NONE:
            return self._no_exploration(obs)
        if self.exploration == QLearning.E_GREEDY:
            return self._epsilon_greedy(obs)
        elif self.exploration == QLearning.BOLTZMANN:
...
if __name__ == "__main__":
    ...
    agent = QLearning(env, alpha=0.3, num_episodes=1500, max_steps=2500)
